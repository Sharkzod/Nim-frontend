...
// Import components (you may need to update these components to remove their API dependencies too)
import ProductDetailPage from '../../components/DropShipPageDetsil';
import ReviewsSectionComponent from '../../components/ReviewsSection';
...
const mockProduct: Product = {
  ...
  sizes: ['S', 'M', 'L', 'XL'],
  bulkPrices: [
    { quantity: 10, price: 27999 },
    { quantity: 50, price: 25999 }
  ],
...
// Mock data for seller's other products
...
const Product = () => {
  ...
  useEffect(() => {
    const loadData = () => {
      ...
      setTimeout(() => {
        ...
        setSellerProducts(mockSellerProducts);
  ...
  const handleWishlistToggle = async (productId: string, currentStatus: boolean) => {
    console.log(`Toggling wishlist for product ${productId}. Current status: ${currentStatus}`);
    
    // Toggle the wishlist status locally
    if (product && product._id === productId) {
      setProduct({
        ...product,
        isWishlisted: !currentStatus
      });
    }
    
    // Update similar products
    setSimilarProducts(prev => prev.map(p => 
      p._id === productId ? { ...p, isWishlisted: !currentStatus } : p
    ));
  ...
  const handleMessageSeller = async () => {
    ...
    alert('This would normally navigate to chat. Using mock data.');
  };

  // Mock buy now function
  ...
  const handlePaymentRedirect = (paymentUrl: string) => {
    ...
    setShowPaymentPopup(false);
    alert('This would redirect to payment gateway. Using mock data.');
  };
      ...
      <div className="lg:hidden w-full">
        <div className="space-y-0">
          ...
          <div className="relative">
            <div className="aspect-square bg-gray-50 select-none">
              <img
                src={product.images && product.images[currentImageIndex] ? product.images[currentImageIndex] : '/placeholder-product.png'}
          ...
          <div className="px-4 py-3">
            ...
            <div className="flex gap-3 pt-4">
              <button 
                ...
                className="flex-1 bg-white text-[#3652AD] border border-[#3652AD] py-2 px-4 rounded-[100px] font-medium text-sm hover:bg-blue-50 transition-colors"
              >
                Message seller
              </button>
              <button 
                onClick={handleBuyNow}
                className="flex-1 bg-[#3652AD] text-white py-2 px-4 rounded-[100px] font-medium text-sm hover:bg-blue-700 transition-colors"
              >
                Buy now
              </button>
            </div>
  ...
  return (
    <div className='w-full'>
      ...
      {/* Reviews Section - You might want to update this component to use mock data too */}
