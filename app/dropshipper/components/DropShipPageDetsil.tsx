...
const ProductDetailPage: React.FC<ProductDetailPageProps> = ({ 
  ...
  useEffect(() => {
    if (propProduct) {
      setLocalProduct(propProduct);
    } else {
      setLocalProduct(mockProduct);
    }
  }, [propProduct]);
  ...
  useEffect(() => {
    ...
    if (localProduct?.colors && localProduct.colors.length > 0) {
      setSelectedColor(localProduct.colors[0].name);
    }
  }, [localProduct]);

  // Generate specifications from product data
  const specifications: ProductSpec[] = localProduct ? [
    { label: 'Type', value: localProduct.type || 'N/A' },
    { label: 'Gender', value: localProduct.gender || 'Unisex' },
    { label: 'Color', value: localProduct.colors?.map(c => c.name).join(', ') || 'Various' },
    { label: 'Size', value: localProduct.sizes?.join(', ') || 'One Size' },
    { label: 'Condition', value: localProduct.condition || 'New' }
  ] : [];
  ...
  const handleShare = () => {
    if (navigator.share) {
      navigator.share({
        title: localProduct?.name || 'Product',
        text: localProduct?.description,
  ...
  const handleWishlistToggle = async () => {
    ...
    try {
      ...
      const newWishlistStatus = !localProduct.isWishlisted;
      setLocalProduct({
        ...localProduct,
        isWishlisted: newWishlistStatus
      });
      
      console.log(`Wishlist status updated to: ${newWishlistStatus}`);
      alert(`Product ${newWishlistStatus ? 'added to' : 'removed from'} wishlist!`);
    } catch (error) {
  ...
  const handleMessageSeller = () => {
    ...
    console.log('Message seller:', localProduct.vendor._id);
    alert(`This would normally open a chat with ${localProduct.vendor.businessName}`);
  };
  ...
  const handleBuyNow = () => {
    ...
    const orderDetails = {
      ...
      total: localProduct.price * quantity
    };
    
    console.log('Buy now:', orderDetails);
    alert(`Added ${quantity} ${localProduct.name} to cart!`);
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
        ...
        <div className="flex gap-4">
          ...
          <div className="flex-1 rounded-lg overflow-hidden">
            <img
              src={localProduct.images[selectedImage]}
        ...
        <div className="space-y-6">
          ...
          <div>
            <div className="flex items-start justify-between mb-2">
              <div className="flex items-center gap-2 text-sm text-gray-600">
                ...
                <span>{localProduct.vendor?.location || 'Location not specified'}</span>
              </div>
              <span className="text-xs text-gray-500">Promoted</span>
            </div>

            <div className="flex items-center gap-2 mb-3">
              <div className="flex">{renderStars(localProduct.rating)}</div>
              {localProduct.rating && (
                <span className="text-sm text-gray-600">({localProduct.rating})</span>
              )}
            </div>

            <h1 className="text-2xl font-bold text-gray-900 mb-2">{localProduct.name}</h1>
            ...
            <div className="flex items-center gap-3 mb-2">
              ...
              {localProduct.originalPrice && localProduct.originalPrice > localProduct.price && (
                <div className="text-lg text-gray-500 line-through">
                  {formatPrice(localProduct.originalPrice)}
                </div>
              )}
            </div>

            {/* Bulk Pricing */}
            {localProduct.bulkPrices && localProduct.bulkPrices.length > 0 && (
              <div className="mt-3 space-y-2 text-sm">
                <div className="font-medium text-gray-700">Bulk Pricing:</div>
                {localProduct.bulkPrices.map((bulkPrice, index) => (
          ...
          <div className="flex items-center justify-end gap-4 pt-4 border-t border-gray-200">
            ...
            >
              <Heart
                className={`w-5 h-5 ${localProduct.isWishlisted ? 'fill-red-500 text-red-500' : ''}`}
              />
