...
export default function PaymentPopup({ 
  ...
  // Mock user balance
  const [userBalance] = useState(45000); // ₦45,000

  // Initialize delivery address from mock user profile
  useEffect(() => {
    setDeliveryAddress({
  ...
  const createMockOrder = async (
    ...
    return { type: 'order', data: { order: mockOrder } };
  };
  ...
  const createMockPaymentRedirect = async (): Promise<RedirectResult> => {
    ...
    return { 
      type: 'redirect', 
      data: { 
        paymentUrl: 'https://payment-gateway-mock.com/process' 
      } 
    };
  };
    ...
    try {
      ...
      if (paymentMethod === 'balance') {
        ...
        if (userBalance < orderDetails.total) {
          throw new Error('Insufficient balance. Please use another payment method.');
        }
        
        // Simulate order creation with balance
        result = await createMockOrder(
        ...
        // Simulate payment gateway redirect for crypto or bank transfer
        result = await createMockPaymentRedirect();
      }
      ...
      if (result.type === 'order') {
        ...
        if (onPaymentRedirect && result.data.paymentUrl) {
          ...
          alert(`This would redirect to payment gateway: ${result.data.paymentUrl}\n(Mock payment - using alert instead of actual redirect)`);
          // In real implementation: window.open(result.data.paymentUrl, '_blank');
          onClose();
        }
      }
            ...
            <div className="space-y-3">
              ...
              <label className="flex items-center justify-between p-4 bg-white rounded-2xl cursor-pointer hover:bg-gray-50 transition-colors">
                ...
                <div className="flex gap-1.5">
                  {/* Mock crypto logos - you can use real ones or SVGs */}
